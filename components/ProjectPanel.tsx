  ...
  return (
    <section className="panel group bg-navy" id={panelId}>
      ...
      <div className="absolute inset-0 z-[1]"
        style={{ background: 'linear-gradient(to top, rgba(6,14,26,.96) 0%, rgba(6,14,26,.2) 45%, rgba(12,29,56,.4) 100%)' }}
      />
      ...
      <div className="absolute inset-0 z-[2] flex flex-col justify-end px-10 pb-14 md:px-12">
        <p className="text-[11px] font-medium tracking-[.28em] uppercase text-green mb-3">
          {project.segment}
        </p>
        <h2 className="font-display font-black text-white leading-[.9] tracking-[-0.01em] whitespace-pre-line"
          style={{ fontSize: 'clamp(40px, 7vw, 88px)' }}>
          {project.title}
        </h2>
        <p className="text-[14px] font-light tracking-[.03em] text-white/50 mt-5 max-w-[480px] leading-[1.75]">
          {project.description}
        </p>
      </div>
    </section>
